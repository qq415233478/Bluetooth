...
public class MainActivity extends Activity {
			...
			if (profile == BluetoothProfile.HEADSET) {
				...
				if (pairedDevices.size() > 0) {
					...
					for (BluetoothDevice device : pairedDevices) {
						Log.e(TAG, "BT Device :" + device.getName() + " , BD_ADDR:" + device.getAddress()); // Print
																											// out
																											// Headset
																											// name
					}
				} else {
					Toast.makeText(mContext, "Could not find a connected Headset, please connect a headset",
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!mBluetoothAdapter.isEnabled()) { // checks if bluetooth is enabled,
												// if not it asks for permission
												// to enable it
	...
	public void SCOSetup(View view) {
		...
		builder.setTitle("Bluetooth PC").setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
